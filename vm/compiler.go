...
// ast node
func TrNode_new(vm *struct TrVM, type TrNodeType, a, b, c OBJ, line size_t) OBJ {
...
TrCompiler *TrCompiler_new(struct TrVM *vm, const char *fn) {
  TrCompiler *c = TR_ALLOC(TrCompiler);
  c.line = 1;
  c.vm = vm;
  c.block = newBlock(c, 0);
  c.reg = 0;
  c.node = TR_NIL;
...
func TrCompiler_compile_node_to_RK(struct *vm, c *TrCompiler, b *Block, n *TrNode, reg int) int {
  int i;
  
  /* k value */
  if (NODE_TYPE(n) == NODE_VALUE) {
    return b.push_value(NODE_ARG(n, 0)) | 0x100;
    
  /* local */
  } else if (NODE_TYPE(n) == NODE_SEND && (i = b.find_local(NODE_ARG(NODE_ARG(n, 1), 0))) != -1) {
    return i;
...
func TrCompiler_compile_node(vm *TrVM, c *TrCompiler, b *Block, n *TrNode, reg int) OBJ {
  if (!n) return TR_NIL;
  int start_reg = reg;
  REG(reg);
  b.line = n.line;
  ...
  switch (n.ntype) {
    ...
    case NODE_BLOCK:
      ...
      break;
    case NODE_VALUE: {
      int i = b.push_value(n.args[0]);
      PUSH_OP_ABx(b, LOADK, reg, i);
    } break;
    case NODE_STRING: {
      int i = b.push_string(TR_STR_PTR(n.args[0]));
    ...
    case NODE_SETIVAR:
      COMPILE_NODE(b, n.args[1], reg);
      PUSH_OP_ABx(b, SETIVAR, reg, b.push_value(n.args[0]));
      break;
    case NODE_GETIVAR:
      PUSH_OP_ABx(b, GETIVAR, reg, b.push_value(n.args[0]));
      break;
    case NODE_SETCVAR:
      COMPILE_NODE(b, n.args[1], reg);
      PUSH_OP_ABx(b, SETCVAR, reg, b.push_value(n.args[0]));
      break;
    case NODE_GETCVAR:
      PUSH_OP_ABx(b, GETCVAR, reg, b.push_value(n.args[0]));
      break;
    case NODE_SETGLOBAL:
      COMPILE_NODE(b, n.args[1], reg);
      PUSH_OP_ABx(b, SETGLOBAL, reg, b.push_value(n.args[0]));
      break;
    case NODE_GETGLOBAL:
      PUSH_OP_ABx(b, GETGLOBAL, reg, b.push_value(n.args[0]));
      break;
    case NODE_SEND:
      ...
      /* method call */
        ...
        if (n.args[2]) {
          ...
          if (blkn.args[1]) {
            ...
            TR_ARRAY_EACH(blkn.args[1], i, v, {
              ...
              blk.push_local(param.args[0]);
            });
          }
          kv_push(Block *, b.blocks, blk);
        ...
        if (blk && kv_size(blk.upvals)) {
          ...
          for(j = 0; j < kv_size(blk.upvals); ++j) {
            ...
            int vali = b.find_local(upval_name);
            if (vali != -1)
      ...
      if (n.args[1]) {
        ...
        TR_ARRAY_EACH(n.args[1], i, v, {
          ...
          if (param.args[2]) {
            ...
            blk.defaults.push(kv_size(blk.code));
          }
          blk_reg++;
        });
      }
    ...
    case NODE_MODULE: {
      ...
      if (n.ntype == NODE_CLASS) {
        /* superclass */
        if (n.args[1])
          PUSH_OP_ABx(b, GETCONST, reg, b.push_value(n.args[1]));
        else
          PUSH_OP_A(b, NIL, reg);
        PUSH_OP_ABx(b, CLASS, blki, b.push_value(n.args[0]));
        PUSH_OP_A(b, BOING, reg);
      } else {
        PUSH_OP_ABx(b, MODULE, blki, b.push_value(n.args[0]));
      }
    } break;
    case NODE_CONST:
      PUSH_OP_ABx(b, GETCONST, reg, b.push_value(n.args[0]));
      break;
    case NODE_SETCONST:
      COMPILE_NODE(b, n.args[1], reg);
      PUSH_OP_ABx(b, SETCONST, reg, b.push_value(n.args[0]));
      break;
    case NODE_ADD:
    case NODE_SUB:
    case NODE_LT: {
